  ...
  confirmBooking() {
    ...
    this.flightService.bookFlight(this.flightId, this.bookingSeatClass()).subscribe({
      next: (response) => {
        ...
        this.bookingId.set(response.bookingId); // Assuming the response contains bookingId
        this.bookingSeatClass.set(response.seatClass);
        console.log('Booking confirmed:', response);
      },
      error: (err) => {
        this.isBooking.set(false);
        let message = 'Failed to book flight';
